...
internal class PitchRendererV2
{
    ...
    public PitchRendererV2(RenderInfoCommon renderInfo)
    {
    ...
    public void Render(SKCanvas g)
    {
        ...
        if (this._renderInfo.Track is not NeutrinoV2Track track)
            return;
        ...
        foreach (var pitchGroup in pitches)
        {
            ...
            if (pointsIdx > 0)
            {
                ...
                g.DrawPoints(SKPointMode.Polygon, origPoints[range], new SKPaint { Color = SKColors.OrangeRed, StrokeWidth = 1.5f, IsAntialias = true });
